...
- **Push notifications (Supabase):** `dispatch-member-push` reads `EXPO_ACCESS_TOKEN` — **same Expo access token value**, different name/system. After local save, run `npm run supabase:sync-expo-push-secret` when `SUPABASE_ACCESS_TOKEN` is set (Supabase dashboard → Account → Access tokens). Never commit either token.
