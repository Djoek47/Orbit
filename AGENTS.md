...
**Canonical branch:** `cursor/make-v20` → PR into `main`.
...
- Lineage: make-v18 (TF **1.3.0 (69)**) → make-v19 (TF **1.3.0 (71)**) → make-v20. See `docs/choremaxx-make-v20.md`.
...
- After clone / on cloud VM: `git checkout cursor/make-v20 && git pull`, then `npm install`, `cp -n .env.example .env`, then use **`npm run start:persistent`** (keep-alive supervisor). Do **not** use bare `expo start` / LAN URLs — phones cannot reach `exp://172…`. The pipeline is:
...
- **Shipped baseline:** `cursor/make-v20` (inherits make-v19 / TF **1.3.0 (71)**). Do not abandon it for other `cursor/*` branches.
