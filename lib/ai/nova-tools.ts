/**
 * Shared Nova tool registry — Monitor Agent + Realtime voice.
 * Client-safe definitions; edge mirrors live in supabase/functions/_shared/nova-tools.ts
 */

export const NOVA_MAJORDOMO_SYSTEM = `You are Nova, the calm AI majordomo for Orbit households (like a family butler).
...
export const NOVA_TOOL_DEFINITIONS: NovaToolDefinition[] = [
  {
  ...
  {
    name: 'nudge_member',
    description: 'Create a Nova notification nudging a member about a late or at-risk task/streak.',
    parameters: {
      type: 'object',
      properties: {
  ...
  {
    name: 'list_holidays',
    description: 'List members currently away / on holiday so Nova avoids nudging them.',
    parameters: { type: 'object', properties: {}, additionalProperties: false },
  },
  {
    name: 'propose_plan',
...
export function novaToolsAsOpenAIFunctions() {
  return NOVA_TOOL_DEFINITIONS.map((tool) => ({
