/**
 * Shared Poppins tool registry — Monitor + Realtime voice + chat.
...
export function orderPoppinsToolCalls<T extends { name: string }>(calls: T[]): T[] {
  const rank = (name: string) => {
    ...
    return 3; // session / end_session last
  };
